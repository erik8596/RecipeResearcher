...
{
    internal class KetoRecipe : RecipeIF
    {
        public async Task GenerateRecipesAsync(int numRecipes)
        {
            await Task.Run(() =>
            {
                ...
                List<string> glutenFreeRecipes = allRecipes.Skip(1).Where(line => line.Contains("Keto")).ToList();
                ...
                MessageBox.Show($"Created {numRecipes} keto recipe(s).");
            });
        }
    }
}
