...
{
    internal class VeganRecipe : RecipeIF
    {
        public async Task GenerateRecipesAsync(int numRecipes)
        {
            await Task.Run(() =>
            {
                ...
                List<string> glutenFreeRecipes = allRecipes.Skip(1).Where(line => line.Contains("Vegan")).ToList();
                ...
                MessageBox.Show($"Created {numRecipes} vegan recipe(s)."); 
            });
        }
    }
}
